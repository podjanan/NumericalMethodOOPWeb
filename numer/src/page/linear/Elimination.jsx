...
function Elimination(){
    ...
    const calculate = () => {
      ...
      const a = Array.from({ length: n }, (_, r) =>
        Array.from({ length: n }, (__ , c) => {
          ...
          return Number.isFinite(v) ? v : 0; // ว่าง/ไม่ใช่ตัวเลข -> 0
        })
      );
      ...
      const EPS = 1e-12;
      for (let i = 0; i < n; i++) {
        // 1) เลือก pivot แถวที่คอลัมน์ i (partial pivoting)
        let pivotRow = i;
        for (let r = i + 1; r < n; r++) {
        ...
        // ถ้า pivot เล็กเกินไป => ไม่มีคำตอบเอกพันธ์ (singular / ill-conditioned)
        if (Math.abs(a[pivotRow][i]) < EPS) {
          setX([]); // ล้างผลลัพธ์
          // จะ alert หรือแค่ return ก็ได้ เลือก return เฉย ๆ ไว้ก่อน
          return;
        }
        // 2) ทำให้ค่าด้านล่าง pivot เป็นศูนย์ (forward elimination)
        for (let r = i + 1; r < n; r++) {
    ...
    return (
      ...
      <h1 className="text-2xl font-bold mb-4">Guass Elimination Methods</h1>
